...
class Login extends React.Component {
  constructor() {
    ...
    this.type = 'token'
    this.scope = `user-read-private user-read-email user-read-playback-state 
    user-library-read user-library-modify user-read-recently-played`
    this.stateID = Math.floor(Math.random() * (9999 - 1000)) + 1000
  }

  handleClick = () => {
    const URL = `https://accounts.spotify.com/authorize?client_id=${this.clientId}&client_secret=
    ${this.clientSecret}&redirect_uri=${this.redirectURI}&scope=
    ${this.scope.split(' ').join('%20')}&response_type=${this.type}&state=${this.stateID}`
